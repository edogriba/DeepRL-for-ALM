...
def evaluate_ppo_alm(
    ...
    """
    Evaluates a trained PPO policy on unseen synthetic trajectories.
    The environment handles early termination and liquidity constraints.
    """
    ...
    for ep in range(eval_episodes):
        ...
        while not done:
            ...
            if verbose_first_episode and ep == 0:
                print(
                    f"Ep 0 | Month {ep_len:2d} | NAV: {nav:8.2f} | "
                    f"LCR: {lcr:5.2f} | Step Reward: {rewards[0]:.2f}"
                )
...
def evaluate_ppo_agent(sac_model, markov_config, K, seed=0, verbose=False):
    """
    Single episode rollout for PPO policy with full environment coupling.
    """
    ...
    for t in range(T):
        ...
        if verbose:
            ...
            bond_labels = (
                [cfg["bond_type"] for cfg in markov_config["bond_configs"]]
                + ["HOLD_CASH"]
            )

            print(
                f"Action (Absolute): "
                f"{ {l: f'{w:.2f}€' for l, w in zip(bond_labels, abs_acts)} }"
            )
        ...
        if terminated:
            ...
            if verbose:
                ...
                print(f"Total Penalty: {info.get('penalty', 0.0):.2f} €")
                print(f"Defaulted: {'YES' if ever_bankrupt else 'NO'}")
